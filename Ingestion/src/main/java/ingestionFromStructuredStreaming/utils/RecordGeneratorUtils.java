...
import ingestionFromStructuredStreaming.model.RecordGeneratorException;
import ingestionFromStructuredStreaming.model.RecordGeneratorKeys;
...
public class RecordGeneratorUtils {
    ...
    private static String dataPath = "Ingestion/src/main/resources/data/";
    private static String[] articles = { "The", "My", "A", "Your", "Their", "Our" };
    private static String[] adjectives = { "", "Great", "Beautiful", "Better", "Worse", "Gorgeous", "Terrific", "Fantastic", "Nebulous", "Colorful", "Terrible", "Natural", "Wild" };
    ...
    private RecordGeneratorUtils() throws RecordGeneratorException {
        try {
            femaleFirstNames = Files.readAllLines(new File(dataPath + "female_first_names.txt").toPath(), Charset.defaultCharset());
        } catch (IOException e) {
        ...
        try {
            maleFirstNames = Files.readAllLines(new File(dataPath + "male_first_names.txt").toPath(), Charset.defaultCharset());
        } catch (IOException e) {
        ...
        try {
            lastNames = Files.readAllLines(new File(dataPath + "last_names.txt").toPath(), Charset.defaultCharset());
        } catch (IOException e) {
    ...
    public static int getIdentifier(List<Integer> identifiers) {
        int i;
        do {
            i = getRandomInt(RecordGeneratorKeys.MAX_ID);
    ...
    public static String getLivingPersonDateOfBirth(String format) {
        int year = cal.get(Calendar.YEAR) - getRandomInt(RecordGeneratorKeys.MAX_AGE);
